...
async function fetchTsetmcIfbEffects() {
  ...
  const all = watchRows
    .map((row) => {
      ...
      const outstandingShares = parseNum(row?.ztd)
      if (!symbol || priceChange == null || priceChange === 0 || outstandingShares == null || outstandingShares <= 0) return null
      ...
      // Index impact ranking is proportional to outstanding shares × closing
      // price change. This reproduces TSETMC's IFB positive/negative ordering.
      const rawImpact = outstandingShares * priceChange
      const effect = Math.round((rawImpact / 1e12) * 100) / 100
      return { symbol, name, effect, priceChange, outstandingShares, rawImpact }
    })
    .filter(Boolean)
  if (!all.length) throw new Error('TSETMC IFB market watch: no usable rows')
  return {
    ok: true,
    source: 'tsetmc-ifb-marketcap-impact',
...
function mergeImpacts(rahavard, board, arena, tsetmcIfb) {
...
export async function onRequestGet(context) {
  ...
  // Collected from the complete official TSETMC IFB universe. Use the
  // deployed daily snapshot only when the live TSETMC request is unavailable;
  // a stale cache must never overwrite a successful live ranking.
  if (
    !tsetmcIfb?.ok &&
    impactsCache?.sources?.some((source) =>
      source === 'tsetmc-official-ifb' ||
      source === 'tsetmc-marketwatch-ifb-percent' ||
      source === 'tsetmc-marketwatch-ifb-impact'
    ) &&
    impactsCache?.ifbPos?.length &&
    impactsCache?.ifbNeg?.length
  ) {
