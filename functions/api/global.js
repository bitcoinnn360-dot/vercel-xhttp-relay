...
export async function onRequestGet(context) {
  ...
  try {
    ...
    if (!payload.stocks.length && staticBundle?.stocks?.length) {
      return new Response(
        JSON.stringify({
          ...staticBundle,
          news: [],
    ...
    if (staticBundle?.stocks?.length) {
      return new Response(
        JSON.stringify({
          ...staticBundle,
          news: [],
          ok: true,
          errors: [String(e?.message || e), ...errors].slice(0, 8),
          served: 'static-fallback',
        }),
        { headers },
      )
    }
    return new Response(JSON.stringify({ ok: false, error: String(e?.message || e), errors }), {
      status: 502,
      headers,
    })
  }
